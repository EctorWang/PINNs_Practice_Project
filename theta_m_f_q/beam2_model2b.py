...
if __name__ == '__main__':
    ...
    plt.plot(X_star[501:-1], u_fem[501:-1], label='Computed Displacement $u_{fem}$', linewidth=2, color='blue')
    plt.xlabel('Position $x$')
    plt.ylabel('Displacement $u_{fem}$')
    plt.title('Computed Displacement vs. Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2b_computed_displacement_vs_position.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()


    # Plot the computed Corner a_fem against the position X_star
    plt.figure(figsize=(10, 4))
    plt.plot(X_star[501:-1], a_fem[501:-1], label='Computed Displacement $u_{fem}$', linewidth=2, color='blue')
    plt.xlabel('Position $x$')
    plt.ylabel('Corner $a_{fem}$')
    plt.title('Computed Corner vs. Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2b_computed_Corner_vs_position.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()

    # Plot the computed Curvature u_fem against the position X_star
    plt.figure(figsize=(10, 4))
    plt.plot(X_star[501:-1], k_fem[501:-1], label='Computed Curvature $u_{fem}$', linewidth=2, color='blue')
    plt.xlabel('Position $x$')
    plt.ylabel('Curvature $k_{fem}$')
    plt.title('Computed Curvature vs. Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2b_computed_Curvature_vs_position.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.savefig('beam2_model2b_training_points_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    X_uc = np.array([[0.5*l], [l]])
    u_c = np.array([[u_fem[501]], [u_fem[-1]]])  # Ensure correct shape
    X_ac = np.array([[0.5*l], [l]])
    a_c = np.array([[a_fem[501]], [a_fem[-1]]])  # Ensure correct shape
    X_kc = np.array([[0.5*l], [l]])
    k_c = np.array([[-k_fem[501]], [-k_fem[-1]]])  # Ensure correct shape

    model = PINN_model(layers, X_star, q, X_uc, u_c, X_ac, a_c, X_kc, k_c)
    model.train(8000)

    Q_pred, k_pred, a_pred, u_pred = model.predict(X_star[501:-1])

    plt.figure(figsize=(10, 2))
    plt.plot(X_star[501:-1], u_fem[501:-1], label='Generated Data', linewidth=2)
    plt.plot(X_star[501:-1], u_pred, '--', label='PINN', linewidth=2)
    plt.legend()
    plt.savefig('beam2_model2b_generated_data_vs_PINN_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2b_training_losses_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
