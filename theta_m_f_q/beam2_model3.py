...
if __name__ == '__main__':
    ...
    # Define constants
    Fx = 1.0  # Example value for force
    EI = 1.0  # Example value for EI
    l = 1.0  # Example value for length

    # Generate data using the new formula w = -Fx(3l^2 - 4x^2) / (48EI)
    # 对称结构，所以只考虑一半
    X_star = np.linspace(0, 0.5 * l, 1001).reshape(-1, 1)
    u_fem = -Fx * X_star * (3 * l**2 - 4 * X_star**2) / (48 * EI)  # Generate displacement data
    a_fem = -Fx * (l**2 - 4 * X_star**2) / (16 * EI)  # Generate corner data
    k_fem = Fx * X_star / (2 * EI)  # Generate curvature data
    ...
    plt.plot([0, 0.5 * l], [0, 0], '^', markersize=10, label='Boundary')
    plt.xlim(-0.05, 0.5 * l + 0.05)
    plt.ylim(-0.7, 2.0)
    plt.plot(X_train, np.zeros(X_train.shape), 'o', markersize=8, alpha=0.5, label='Random data')
    plt.yticks([])
    plt.xlabel('Location x')
    plt.legend(fontsize=12, ncol=2)
    plt.savefig('beam2_model3_training_points_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()

    q = np.full_like(X_train, 0)
    X_uc = np.array([[0.0], [0.5 * l]])
    u_c = np.array([[u_fem[0, 0]], [u_fem[-1, 0]]])   # Displacement at the endpoint
    X_ac = np.array([[0.0], [0.5 * l]])
    a_c = np.array([[a_fem[0, 0]], [a_fem[-1, 0]]])  # Corner at the endpoint
    X_kc = np.array([[0.0], [0.5 * l]])
    k_c = np.array([[-k_fem[0, 0]], [-k_fem[-1, 0]]])  # Curvature at the endpoint
    ...
    plt.legend()
    plt.savefig('beam2_model3_generated_data_vs_PINN_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model3_training_losses_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show() 