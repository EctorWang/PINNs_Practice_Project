...
if __name__ == '__main__':
    ...
    M_c = 1.0  # Example value for M_c

    # Generate data using the new formula w = -M_cx(l^2-x^2) / (6EI)
    X_star = np.linspace(0, l, 1001).reshape(-1, 1)
    u_fem = -M_c * X_star * (l**2 - X_star**2) / (6 * EI * l)  # Generate displacement data
    a_fem = -M_c * (l**2 - 3 * X_star**2) / (6 * EI * l)  # Generate corner data
    k_fem = M_c * X_star / (EI * l)  # Generate curvature data
    ...
    plt.savefig('beam2_model1_training_points_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.savefig('beam2_model1_generated_data_vs_PINN_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model1_training_losses_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()