...
if __name__ == '__main__':
    ...
    # Define constants
    F = 1.0  # Example value for force
    EI = 1.0  # Example value for EI
    l = 1.0  # Example value for length

    # Generate data using the formula w = -Fx^2(3l-x) / (6EI)
    X_star = np.linspace(0, l, 1001).reshape(-1, 1)
    u_fem = -F * X_star**2 * (3 * l - X_star) / (6 * EI)  # Generate displacement data
    a_fem = -F * X_star * (2 * l - X_star) / (2 * EI)  # Generate corner data
    k_fem = -F * (l - X_star) / (EI)  # Generate curvature data
    ...
    plt.savefig('beam1_model2_training_points_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.savefig('beam1_model2_generated_data_vs_PINN_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('beam1_model2_training_losses_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
