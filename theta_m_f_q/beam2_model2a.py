...
if __name__ == '__main__':
    ...
    plt.plot(X_star[0:500], u_fem[0:500], label='Computed Displacement $u_{fem}$', linewidth=2, color='blue')
    plt.xlabel('Position $x$')
    plt.ylabel('Displacement $u_{fem}$')
    plt.title('Computed Displacement vs. Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2a_computed_displacement_vs_position.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()


    # Plot the computed Corner a_fem against the position X_star
    plt.figure(figsize=(10, 4))
    plt.plot(X_star[0:500], a_fem[0:500], label='Computed Displacement $u_{fem}$', linewidth=2, color='blue')
    plt.xlabel('Position $x$')
    plt.ylabel('Corner $a_{fem}$')
    plt.title('Computed Corner vs. Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2a_computed_Corner_vs_position.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()

    # Plot the computed Curvature u_fem against the position X_star
    plt.figure(figsize=(10, 4))
    plt.plot(X_star[0:500], k_fem[0:500], label='Computed Curvature $u_{fem}$', linewidth=2, color='blue')
    plt.xlabel('Position $x$')
    plt.ylabel('Curvature $k_{fem}$')
    plt.title('Computed Curvature vs. Position')
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2a_computed_Curvature_vs_position.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.savefig('beam2_model2a_training_points_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()

    # q = np.full_like(X_train, 0)
    q_fem = np.zeros_like(X_star)
    for i, x in enumerate(X_star):
        if x <= a:
            q_fem[i] = -M_e / (l)
            ...
            q_fem[i] = -M_e / (l)
    q = np.full_like(X_star, -q_fem[0])
    X_uc = np.array([[0.0], [0.5*l]])
    u_c = np.array([[u_fem[0]], [u_fem[500]]])  # Ensure correct shape
    X_ac = np.array([[0.0], [0.5*l]])
    a_c = np.array([[a_fem[0]], [a_fem[500]]])  # Ensure correct shape
    X_kc = np.array([[0.0], [0.25*l]])
    k_c = np.array([[-k_fem[0]], [-k_fem[500]]])  # Ensure correct shape

    model = PINN_model(layers, X_star, q, X_uc, u_c, X_ac, a_c, X_kc, k_c)
    model.train(8000)

    Q_pred, k_pred, a_pred, u_pred = model.predict(X_star[0:500])

    plt.figure(figsize=(10, 2))
    plt.plot(X_star[0:500], u_fem[0:500], label='Generated Data', linewidth=2)
    plt.plot(X_star[0:500], u_pred, '--', label='PINN', linewidth=2)
    plt.legend()
    plt.savefig('beam2_model2a_generated_data_vs_PINN_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('beam2_model2a_training_losses_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
