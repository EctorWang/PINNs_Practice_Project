...
if __name__ == '__main__':
    ...
    # Generate data using the new formula w = -qx^2(3l-x) / (6EI)
    X_star = np.linspace(0, l, 1001).reshape(-1, 1)
    u_fem = -q * X_star**2 * (X_star**2 - 4 * l * X_star + 6 * l**2) / (24 * EI)  # Generate displacement data
    a_fem = -q * X_star * (X_star**2 - 3 * l * X_star + 3 * l**2) / (6 * EI)  # Generate corner data
    k_fem = -q * (X_star**2 - 2 * l * X_star + l**2) / (2 * EI)  # Generate curvature data
    ...
    plt.savefig('beam1_model3_training_points_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()

    q = np.full_like(X_train, -q)
    X_uc = np.array([[0.0], [l]])
    u_c = np.array([[u_fem[0, 0]], [u_fem[-1, 0]]])   # Displacement at the endpoint
    X_ac = np.array([[0.0], [l]])
    a_c = np.array([[a_fem[0, 0]], [a_fem[-1, 0]]])  # Corner at the endpoint
    X_kc = np.array([[0.0], [l]])
    k_c = np.array([[-k_fem[0, 0]], [-k_fem[-1, 0]]])  # Curvature at the endpoint 其实是弯矩大小除以EI的值
    ...
    plt.legend()
    plt.savefig('beam1_model3_generated_data_vs_PINN_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()
    ...
    plt.legend()
    plt.grid(True)
    plt.savefig('beam1_model3_training_losses_high_res.png', dpi=300, bbox_inches='tight')  # Save the plot
    plt.show()